...
import { getMockStats, getMockFeedback, getMockTeams, getMockTags, getMockSurveysByApp } from '~/lib/mockData'
...
function handleMockRequest(path: string, params: URLSearchParams): Response | null {
  ...
  if (path.includes('api/v1/intern/export')) {
    ...
    console.log('[Mock] Exporting', feedbackData.content.length, 'items as', format)
    ...
    const headers = [
      ...
      ...fieldIds.map(id => {
        ...
        const typeLabel = field.type === 'RATING' ? ' (vurdering)' : field.type === 'TEXT' ? ' (tekst)' : ''
    ...
    items.forEach(item => {
      const row: string[] = [
        ...
        item.app || '',
        item.surveyId || '',
        item.submittedAt ? new Date(item.submittedAt).toLocaleString('no-NO') : '',
      ]
