...
import { Skeleton, BodyShort } from "@navikt/ds-react";
...
export function RatingChart() {
  ...
  if (total === 0) {
    return (
      <div style={{ display: "flex", alignItems: "center", justifyContent: "center", height: "100%", color: CHART_STYLES.textMuted }}>
  ...
  return (
    <div style={{ height: "100%", display: "flex", flexDirection: "column" }}>
      {questionLabel && (
        <BodyShort size="small" style={{ color: CHART_STYLES.text, marginBottom: "0.5rem" }}>
          «{questionLabel}»
        </BodyShort>
      )}
      <div style={{ flex: 1 }}>
            ...
            if (active && payload && payload.length) {
              ...
              return (
                <div style={{
                  ...
                  <div style={{ fontSize: "1.5rem", marginBottom: "0.25rem" }}>{data.emoji}</div>
                  <div style={{ fontWeight: 600 }}>{data.count.toLocaleString("no-NO")} svar</div>
                  <div style={{ color: CHART_STYLES.textMuted, fontSize: "0.875rem" }}>{percentage}%</div>
                </div>
              );
            }
            return null;
