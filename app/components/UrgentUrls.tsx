...
export function UrgentUrls() {
    ...
    if (!stats?.lowestRatingPaths || Object.keys(stats.lowestRatingPaths).length === 0) {
        return null;
    }
    ...
    return (
        <div className="dashboard-card" style={{ gridColumn: "span 2" }}>
            <div className="dashboard-card-header">
                <Heading size="small" style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                ...
                <Table.Body>
                    {urls.map((row) => (
                        <Table.Row key={row.path}>
                            <Table.DataCell>
                                <a
                                    href={row.path.startsWith("http") ? row.path : `https://www.nav.no${row.path}`}
                                    target="_blank"
                                    rel="noreferrer"
                                    style={{ textDecoration: 'none', color: 'var(--a-text-action)' }}
                                >
                                    {row.path}
                                </a>
                            </Table.DataCell>
