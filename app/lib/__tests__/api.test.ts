...
import type { Answer, FieldType, RatingAnswer, TextAnswer, SingleChoiceAnswer } from "../api";
...
describe("API Types", () => {
  describe("Answer type discrimination", () => {
    ...
    it("can discriminate Answer union type by fieldType", () => {
      ...
      const ratings = answers.filter((a): a is RatingAnswer => a.fieldType === "RATING");
      const texts = answers.filter((a): a is TextAnswer => a.fieldType === "TEXT");
  ...
  describe("FieldType enum values", () => {
    it("has expected field types", () => {
      const fieldTypes: FieldType[] = ["RATING", "TEXT", "SINGLE_CHOICE", "MULTI_CHOICE", "DATE"];
